...
if __name__ == '__main__':
    ...
    if nproc == 1:
        cdo_compute_time, cdo_total_time = compute_cdo(
            filepath=filepath, weightspath=weightspath, targetgridfile=targetgridfile,
            loglevel=loglevel
        )

    retrieve_time, compute_time, io_time, total_time = compute_aqua(
        catalog=catalog, model=model, exp=exp, source=source, nproc=nproc, loglevel=loglevel
    )
